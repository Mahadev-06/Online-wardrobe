    ...
    <div className="min-h-screen bg-p_dark text-white flex flex-col page-enter relative overflow-hidden">
      ...
      <nav className="p-4 md:p-6 flex justify-between items-center max-w-7xl mx-auto w-full relative z-10">
        <div className="flex items-center gap-2 group cursor-default">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-p_teal rounded-xl flex items-center justify-center text-white font-cotta text-xl md:text-2xl shadow-lg transform rotate-3 pt-1 transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110">W</div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
            ...
            <button 
                onClick={onSignup}
                className="px-4 md:px-6 py-2 md:py-2.5 bg-white/20 backdrop-blur-md border border-white/30 text-white font-bold rounded-lg hover:bg-white/30 transition-all shadow-lg btn-hover hover:scale-105 text-sm md:text-base"
            >
                Get Started
            </button>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center p-6 max-w-5xl mx-auto mt-4 md:mt-12 relative z-10">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-white/5 border border-white/10 text-white rounded-full text-xs md:text-sm font-bold mb-6 md:mb-8 backdrop-blur-sm animate-pulse cursor-default hover:bg-white/10 transition-colors">
        ...
        <div className="flex flex-col sm:flex-row gap-4 md:gap-6 w-full justify-center px-6">
            <button 
                onClick={onSignup}
                className="group px-8 md:px-10 py-4 md:py-5 bg-p_teal/20 backdrop-blur-md border border-p_teal/30 text-p_teal rounded-xl font-bold text-lg md:text-xl shadow-2xl hover:shadow-p_teal/20 hover:bg-p_teal/30 transition-all btn-hover flex items-center justify-center gap-2 hover:scale-105 w-full sm:w-auto"
            >
...
const FeatureCard: React.FC<{icon: React.ReactNode, title: string, desc: string}> = ({ icon, title, desc }) => (
    <div className="group bg-gray-800/30 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-white/10 text-left hover:scale-[1.02] transition duration-300 shadow-xl cursor-default hover:shadow-2xl">
        <div className="w-12 h-12 md:w-16 md:h-16 bg-gray-700/50 backdrop-blur-md border border-white/10 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-sm transition-colors group-hover:bg-gray-600/50">
            {icon}
        </div>
        <h3 className="font-black text-white text-lg md:text-xl mb-2 md:mb-3 tracking-wide">{title}</h3>
